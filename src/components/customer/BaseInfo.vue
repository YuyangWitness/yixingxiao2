	...
	<div id="baseinfo">
			<form class="mui-input-group form1" id="BasicForm">
				...
				<div class="mui-input-row">
					<label>客户名称</label>
					<input type="text" id="customerName" name="customerName" @blur="changeCusName({e:$event,msg:'customerName'})">
				</div>
				...
				<div class="mui-input-row">
					...
					<input class="input1" type="number" id="Revenue" name="Revenue" @blur="changeBasicNumber({e:$event,msg:'BasicNumber'})">
					<label class="label2">万元</label>
				</div>
				<div class="mui-input-row">
					<label class="label1">工资总额</label>
					<input class="input1" type="number" id="Total" name="Total" @blur="changeBasicNumber({e:$event,msg:'BasicNumber'})">
					<label class="label2">万元</label>
				</div>
				<div class="mui-input-row">
					<label class="label1">企业人数</label>
					<input class="input1" type="number" id="NumberPeople" name="NumberPeople" @blur="changeBasicNumber({e:$event,msg:'BasicNumber',Integer:1,positive:1})">
...
import { customer } from './../../store/types.js'
export default{
	computed: {
		...mapGetters({
			"customerName": customer.c_getters.basicName
		})
	},
	methods: {
		...mapActions({
			"changeCusName": customer.c_actions.validateBaseInfo,
			"changeBasicNumber": customer.c_actions.validateBaseInfo
		})
